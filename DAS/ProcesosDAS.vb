...
Public Class ProcesosDAS
    ...
    Public Shared Function listarActivos() As List(Of EEProcesos)
        Dim ListaProcesos As List(Of EEProcesos) = Nothing
        Dim loProcesos As EEProcesos = Nothing
        Dim dr As IDataReader = Nothing
        Try
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim cmd As DbCommand = db.GetStoredProcCommand("Listar_Procesos_Activos_pa")
            dr = db.ExecuteReader(cmd)
            While dr.Read()
                If ListaProcesos Is Nothing Then
                    ListaProcesos = New List(Of EEProcesos)()
            ...
            dr.Close()
        Catch ex As Exception
            Throw New Exception("Listar Procesos Activos => " + ex.Message, ex)
        End Try
        Return ListaProcesos
    End Function
