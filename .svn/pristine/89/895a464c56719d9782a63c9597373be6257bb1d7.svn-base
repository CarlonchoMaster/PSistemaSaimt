...
Public Class DiarioPEMDAS
    ...
    Public Shared Function ListarRinconadaXPreTrasDelDia(ByVal fecha As Date) As List(Of EEDiarioPEM)
        Dim ListaDiarioPEM As List(Of EEDiarioPEM) = Nothing
        Dim loDiarioPEM As EEDiarioPEM = Nothing
        Dim dr As IDataReader = Nothing
        Try
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim cmd As DbCommand = db.GetStoredProcCommand("Listar_DiarioRinconadaPreTras_DelDia_pa")
            db.AddInParameter(cmd, "diaFechaIng", DbType.Date, fecha)
            dr = db.ExecuteReader(cmd)
            While dr.Read()
                If ListaDiarioPEM Is Nothing Then
                    ListaDiarioPEM = New List(Of EEDiarioPEM)
            ...
            End While
        Catch ex As Exception
            Throw New Exception("Listar del Día => " + ex.Message, ex)
        End Try
        Return ListaDiarioPEM
    End Function
    ...
    Public Shared Function RinconadaGenerarCodigo() As String
        Dim diaCodigo As String = String.Empty
        ' Dim dr As IDataReader = Nothing
        Try
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim cmd As DbCommand = db.GetStoredProcCommand("Genera_Codigo_DiaRinconada")
            diaCodigo = db.ExecuteScalar(cmd)
            ' dr.Close()
        Catch ex As Exception
