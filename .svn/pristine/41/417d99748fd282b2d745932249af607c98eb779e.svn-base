                ...
                If chkInteresado.Checked Then
                    ...
                    Else
                        ...
                        If txtinteresado.EditValue.Length > 0 Then
                            _SQlString = _SQlString & String.Format(" (YEAR(docFechaIng)={2}) AND  ISNULL(per.perRazonSocial,per.perApePaterno+' '+per.perApeMaterno+' '+ per.perNombres) like '%{0}%' {1} ", txtinteresado.EditValue, cmbInteresado.Text, CboCriterioAnio.EditValue)
                        End If
...
#Region "Reporte Movimientos X Documento"
    Private Sub llenarExcel()
        ...
        Dim loDocumentoSTD As EEDocumentoSTD = MantenimientosBL.Instancia.documentoSTDMostrarXdocId(dtTablaDocumento.Rows(Me.gvResultadoDoc.GetSelectedRows(0))("docInterno").ToString())
        ...
        xl.Range("B15").Value = loDocumentoSTD.perNombreOrigen
        ...
        If loMovimientoSTD IsNot Nothing Then
            ...
            For Each fMovimientoSTD As EEMovimientoSTD In loMovimientoSTD
                ...
                xl.Range("B" & liFila).Value = fMovimientoSTD.OPersonaOrigen.PerNombreCompleto
                xl.Range("D" & liFila).Value = fMovimientoSTD.OPersonaOrigen.OPerCargoPersonal.Cargo
                xl.Range("F" & liFila).Value = fMovimientoSTD.OPersonaDestino.PerNombreCompleto
                xl.Range("H" & liFila).Value = fMovimientoSTD.OPersonaDestino.OPerCargoPersonal.Cargo
                ...
                xl.Range("M" & liFila).Value = fMovimientoSTD.movDiasTenencia
